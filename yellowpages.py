...
def main():
	...
	for city in cityList:
		for term in searchTerms:
			...
			firefox.get("http://www.yellowpages.com/")

			try:
				typeName = firefox.find_element_by_id("query").clear()
				typeName = firefox.find_element_by_id("query")
				print "Set Type : TRUE"
			...
			try:
				loc = firefox.find_element_by_id("location").clear()
				loc = firefox.find_element_by_id("location")
			...
			try:
				URL = firefox.find_elements(By.XPATH,'//li[@class="result"]')
				try:
					...
					while(crawl):
						pageNo = pageNo + 1
						try:
							results = firefox.find_elements(By.XPATH,'//div[@class="result"]')
						except Exception, e:
							print e
						...
						try:
							for result in results:
								...
								try:
									address = soup.find("p", class_="adr").get_text(",", strip=True).split(",")
									#print address
									try:
								...
								try:
									phone = soup.find("div", itemprop="telephone").get_text(" ", strip=True)
									#print phone
								except Exception, e:
									print e
									phone = ""
						...
						try:
							#time.sleep(2)
							page = firefox.find_elements(By.XPATH,'//a[@class="next ajax-page"]')
							if len(page)>0:
								#page[0].click()
			...
			ws = wb.active
			ws.title = term
			excelFileName = "yP - " + term + ", " + city + ".xlsx"
