...
def main():
	...
	for city in cityList:
		for term in searchTerms:
			...
			try:
				URL = firefox.find_elements(By.XPATH,'//ul[@id="results-list"]')
				try:
					...
					while(crawl):
						pageNo = pageNo + 1
						try:
							results = firefox.find_elements(By.XPATH,'//div[@class="biz-listing-large"]')
						except Exception, e:
							print e
						...
						try:
							for result in results:
								...
								try:
									businessName = soup.find("span",class_="biz-name").get_text(" ", strip=True).split(".")[1].strip()
									#print businessName
